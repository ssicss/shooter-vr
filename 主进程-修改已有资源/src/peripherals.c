...
void initSensorGyro(uint16_t gyroScale) {
	...
	tempValue = 0x01;	//No INT1, No boot status on INT1 pin, INT1 inactive, push-pull, No DRDY on INT2 pin, No INT2 FIFO watermakr, No INT2 FIFO overrun, INT2 FIFO empty = on
	L3GD20_Write(&tempValue, L3GD20_CTRL_REG3_ADDR, 1);

	tempValue = 0x40;	//No reboot, Enable FIFO, Disable HPF, INT1 and OUTPUT directly connected to ADC without filters
	L3GD20_Write(&tempValue, L3GD20_CTRL_REG5_ADDR, 1);

	tempValue = 0x40;	//FIFO stream mode, Watermrk level = 0
...
void doGyroBiasCalibration(void){
	float avgBuffers[3];
...
void restoreCalibrationFromFlash(void){
...
void initSensorAccMag( uint8_t accelerometerScale, uint16_t magnetometerScale ) {
	...
	tempValue = 0x10;	//INT1 Click Off, INT1 AOI1 Off, INT1 AOI2 Off, INT1 DRDY1 ON, INT1 DRDY2 Off, INT1 FIFO watermakr Off, INT1 FIFO overrun OFF
	LSM303DLHC_Write(ACC_I2C_ADDRESS, LSM303DLHC_CTRL_REG3_A, &tempValue);

	tempValue = 0x40;	//No reboot, FIFO On, No Latch interrupt, No 4D (No idea what that is)
	LSM303DLHC_Write(ACC_I2C_ADDRESS, LSM303DLHC_CTRL_REG5_A, &tempValue);

	tempValue = 0x80;	//FIFO mode stream, Trigger INT1?, FTH = 0
...
uint8_t getShortLongButtonPress(void){
