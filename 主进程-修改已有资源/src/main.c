...
void initDefaultRiftSettings(void);
void packFoculusdataBlock(void);
void packSensorDataBlock(int32_t Xin, int32_t Yin, int32_t Zin, uint8_t *sensorDataOut);
void handleConfigPacketEvent(void);
...
void handleButton(){
	...
	if( i==1 ){									//Short button press
		...
		STM_EVAL_LEDOn( (Led_TypeDef)orientationIndex );
		while( STM_EVAL_PBGetState( BUTTON_USER ) ); //Wait for button release
		while( 1 ){
			STM_EVAL_LEDOn( (Led_TypeDef)orientationIndex );	//Indicate current orientation index
			i = getShortLongButtonPress();
...
int main(void) {
	...
	//EEPROM_WriteData(0x4,buf2,2);
	printf("start write to tc\n");
	...
	printf("write to tc complete\n");
	for(;;)
	{

		EEPROM_ReadData(0x0,buf,40);
		for(i=0; i<10; i++)
		{
			printf("Addr[0x%2x]:0x%8x\n",0x0+i*4,buf[i]);			
		}
		while(1);
